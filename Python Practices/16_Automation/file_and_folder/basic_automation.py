...
def main():
    ...
    # Start watcher
    observer = Observer()
    observer.schedule(WatcherHandler(), MAIN_FOLDER, recursive=True)
    observer.start()
    log("Folder watcher started")

    # Start background threads
    t1 = threading.Thread(target=backup_scheduler, args=(stop_event,), daemon=True)
    t2 = threading.Thread(target=sync_scheduler, args=(stop_event,), daemon=True)
    t3 = threading.Thread(target=hotkey_listener, args=(stop_event,), daemon=True)
